...
class Server:
    ...
    def indexed_dataset(self) -> Dict[int, List]:
        """Creates an indexed dataset by sorting position.

        Returns:
            Dict[int, List]: The dataset indexed by position starting at 0.
        """
        if self.__indexed_dataset is None:
            dataset = self.dataset()
            truncated_dataset = dataset[:1000]  # Limit to 1000 entries
    ...
    def get_hyper_index(self, index: int = None, page_size: int = 10) -> Dict:
        """Retrieves page info from a given index with a specified size.
